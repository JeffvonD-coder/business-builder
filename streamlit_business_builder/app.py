...
def secure_main():
    """Main function for the secure Streamlit app"""
    ...
    with st.container():
        ...
        st.session_state.selected_language = st.radio(
            UI_TRANSLATIONS["en"]["select_language"],
            options=["English", "Dutch"],
            horizontal=True,
            index=0 if st.session_state.selected_language == "English" else 1
        )
        lang_code = "nl" if st.session_state.selected_language == "Dutch" else "en"
    ...
    # Sidebar with logout and admin menu
    with st.sidebar:
        st.write(f'{texts["welcome"]} *{st.session_state["name"]}*')
        if st.button(texts["logout"], 'sidebar'):
    ...
    # Main app content in a container
    with st.container():
        st.title(texts["title"])
        ...
        if st.button(texts["analyze_button"]):
            ...
            try:
                # Run the analysis
                txt_file, pdf_file = run_business_builder(business_idea, lang_code)
